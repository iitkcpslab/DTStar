...
pair<bool,int> plan_within_horizon(cycle_node* new_origin,ull from,ull till,cycle_node* prev)
{
 ...
 while(!fptr.eof())
 {
   ...
   fptr>>word;
   fptr2<<word<<endl;
   if(!fptr.eof()){fptr>>word;    fptr2<<word<<endl;}
   if(!fptr.eof() )
...
int main(int args,char **argv)
{
    ...
    for(auto i:all_suf_cycle)
    {
       ...
       else if(i.second.first<suff_best)
       {
         pref_cost=cost;
   ...
   else
   {
	...
	while(global_ts<plan_till)
	{
	  ...
	  while(global_ts<check_time && pwh_plan.size()!=0)
	  {
	    ...
	    if(ts>=plan_till)
            { 
              if(ts==plan_till && is_dest(cur) && prev!=NULL && prev_f==cur )
              {
	  ...
	  while(global_ts>=dec_time)
	    ...
	    { 
		  ...
		  if(check_time<=global_ts && check_time!=plan_till)
		  {
