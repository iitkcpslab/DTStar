...
void initializegrid(char *grid_info_file)
{
    ...
    for(i=0;i<no_of_trans;i++)
    {
        ...
        for(j=0;j<trans_condn_len;j++)
        {
            ...
            // PP - significance of strict negative and negative literals 
            ifile>>literal;
...
int main(int args,char **argv)
{
    ...
    for(auto i:all_suf_cycle)
    {
       ...
       if(i.second.first<=pref_best)
       {
         if(i.second.first==pref_best && cost<pref_cost)
         {     
               pref_cost=cost;       
               current_best=i.first;
               pref_best=i.second.first;
               static_pref.clear();
       	       for(auto j:pref_path)
        	static_pref.push_back(j);
         
         }
         else if(i.second.first<pref_best)
         {
           pref_cost=cost;
           current_best=i.first;
           pref_best=i.second.first;
   ...
   else
   {
	...
	while(global_ts<plan_till)
	{
	  ...
	  else if(ts>check_time)// If ts is greater than check_time i.e. we need to get in between location
	  {  
	    if(ts>plan_till)
	    {
		...
		else if(global_ts+suf_dy_path[i].first>check_time)
		{
		  ...
		  break;
	        ...
	        {
	        res<<"Moving from "<<suf_dy_path[i-1].second->x<<","<<suf_dy_path[i-1].second->y<<","<<suf_dy_path[i-1].second->state<<" to "<<suf_dy_path[i].second->x<<","<<suf_dy_path[i].second->y<<","<<suf_dy_path[i].second->state<<" at ts = "<<global_ts+suf_dy_path[i].first<<endl;
	        prev=suf_dy_path[i].second;
	        }
	      }
	  ...
	  if(check_time==obs_ts)
	  {
	    ...
	    if(check_time==global_ts ||replan)
	    { 
		   ...
		   cycle_node* cur_best;
		   float pref_cost;
		   float best_pref=FLT_MAX;
		   float best_suff=FLT_MAX;
		   for(auto i:graph_root)
		   {
		       pref_cost=get_dy_pref(new_origin,i.second,check_time);
		       if(pref_cost==FLT_MAX)
		       continue;

		       float suff_cost=get_dy_suff(i.second,check_time+pref_cost,pref_cost);
		       float cy_len=0;

		       if(suff_cost==FLT_MAX)
		       continue;
		       if(suff_cost<=best_suff)
		       {   
		           if(suff_cost<best_suff || (suff_cost==best_suff && pref_cost<best_pref))
		           {
		             pwh_plan.clear();
		             
		             if(pref_cost==0)
		             pwh_plan.insert(make_pair(check_time,new_origin));
		             
		             else
		             for(auto i:pwh_pref)
		             pwh_plan.insert(make_pair(check_time+i.first,i.second));              
		             best_suff=suff_cost;
		             best_pref=pref_cost;
		             cur_best=i.second;
		           }
		   ...
		   ull time=check_time+best_pref+best_suff;
		   float cost;
		   pwh_plan.insert(make_pair(time,cur_best));
                   check_time=min({plan_till,obs_ts,dec_time});
		   
		   
		   for(;time<=check_time;time+=cost)
		   {
		     cost=get_dy_suff(cur_best,time,time);
		     if(cost==FLT_MAX)
		     break;
                     pwh_plan.insert(make_pair(time+cost,cur_best));
